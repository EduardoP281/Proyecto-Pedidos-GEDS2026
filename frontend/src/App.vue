  ...
  <div class="flex flex-col min-h-screen bg-background text-on-surface font-body-md antialiased selection:bg-blue-600 selection:text-white">
    ...
    <header class="flex justify-between items-center w-full px-6 py-3 border-b border-slate-200 sticky top-0 z-40 bg-white/95 backdrop-blur-md shadow-sm">
      ...
      <div class="flex items-center gap-3">
        ...
        <button class="p-2 rounded-lg text-slate-500 hover:bg-slate-100 hover:text-slate-800 transition-all border border-transparent hover:border-slate-200" @click="showToast('Notificaciones del sistema', 'info')">
...
const checkout = async () => {
  ...
  try {
    const payload = {
      ...
      customerEmail: 'cliente@local.test',
      items: cart.value.map((item) => ({ productId: item.id, quantity: item.quantity })),
    };

    const response = await api.createOrder(payload);
    if (!response || !response.success) {
      throw new Error(response?.error?.message || 'Error al procesar el pedido');
    }
    ...
    showToast(error.message || 'No se pudo procesar el pedido', 'error');
  }
};
